locals {
  ...
  live_blob_accountkey  = module.kube_workloads.live_blob_account_key
  ...
  live_github_token = module.kube_workloads.github_token
...
resource "kubernetes_secret_v1" "live-flo-all-secrets" {
  wait_for_service_account_token = true

  metadata {
    name      = "live-flo-all-secrets"
    namespace = kubernetes_namespace_v1.live-flo.metadata[0].name
    labels = merge(
      {
        "app"                         = "shared"
        "environment"                 = "live"
  ...
  data = {
    ...
    "azuread.tennant_id"     = module.kube_workloads.live_azuread_tenant_id
    "azureb2c.client_id"     = module.kube_workloads.live_azuread_client_id
    "azureb2c.client_secret" = module.kube_workloads.live_azuread_client_secret
    ...
    "cronJobs.apiKey"         = module.kube_workloads.live_cronjobs_api_key
    ...
    "govuk.notify_apikey" = module.kube_workloads.live_govuk_notify_apikey

    "onelogin.client_id"   = module.kube_workloads.live_onelogin_client_id
    "onelogin.private_key" = module.kube_workloads.live_onelogin_private_key
    ...
    # RabbitMQ
    "rabbitmq.username"      = module.kube_workloads.live_rabbitmq_username
    "rabbitmq.password"      = module.kube_workloads.live_rabbitmq_password
    "rabbitmq.erlang_cookie" = module.kube_workloads.live_rabbitmq_erlang_cookie
  }

  type = "Opaque"

  depends_on = [
    kubernetes_namespace_v1.live-flo
  ]

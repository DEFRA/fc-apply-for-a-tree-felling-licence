...
resource "azurerm_private_dns_zone_virtual_network_link" "database" {
  ...
  virtual_network_id    = data.terraform_remote_state.platform.outputs.virtual_network_id
  resource_group_name   = data.terraform_remote_state.platform.outputs.resource_group_name
}
...
resource "azurerm_postgresql_flexible_server" "migrate" {
  ...
  geo_redundant_backup_enabled  = true
  administrator_login           = "qxlva"
  administrator_password        = module.shared.pg_migrate_pass
  zone                          = "1"
  public_network_access_enabled = false
...
resource "azurerm_postgresql_flexible_server" "live" {
  ...
  geo_redundant_backup_enabled  = true
  administrator_login           = "qxlva"
  administrator_password        = module.shared.pg_live_pass
  public_network_access_enabled = false

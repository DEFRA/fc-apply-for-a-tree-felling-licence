...
const LOCAL_DEBUG_BASE_URL = 'https://localhost:7253';
...
const KNOWN_APPLICATION_ID_IN_SUBMITTED_STATE = '940f3d72-06bf-4e42-9829-fd4c4b61e14d';
...
test.beforeAll(async ({ playwright, baseURL }) => {
  apiContext = await playwright.request.newContext({
    ...
    baseURL: baseURL, //LOCAL_DEBUG_BASE_URL, //baseURL, 
    extraHTTPHeaders: {
...
test('cannot add LIS doc when FLA has incorrect state to accept it', async ({ page }) => {
  ...
  const response = await apiContext.put(`/api/lis/${KNOWN_APPLICATION_ID_IN_SUBMITTED_STATE}`, {
      multipart: {
        name: 'filename',
        mimeType:'application/pdf',
        buffer : PDF_FILE_STREAM

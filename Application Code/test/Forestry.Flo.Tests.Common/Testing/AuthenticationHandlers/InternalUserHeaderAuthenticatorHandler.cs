...
/// </summary>
public class InternalUserHeaderAuthenticatorHandler : AuthenticationHandler<HeaderAuthenticationOptions>
{
    ...
    /// <inheritdoc />
    public InternalUserHeaderAuthenticatorHandler(
    ...
    protected override Task<AuthenticateResult> HandleAuthenticateAsync()
    {
        ...
        if (Request.Headers.ContainsKey(headerKey))
        {
            ...
            var testUserAccount = UserFactory
                .CreateInternalUserIdentityProviderClaimsPrincipal(
                    identityProviderId: testUserIdentityProvider,
                    localAccountId: Guid.Parse(testLocalUserIdValue));

            _logger.LogInformation("Creating FC Internal authenticated user with username {Username}", testUserAccount.Identity.Name);

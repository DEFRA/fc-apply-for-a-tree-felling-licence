...
public class RunFcInternalUserConstraintCheckUseCaseTests
{
    ...
    private readonly InternalUser _user;
    private readonly LandInformationSearchOptions _settings = new() { DeepLinkUrlAndPath = "http://www.tempuri.org/abc", LisConfig = "someValue", TargetEnvironment = LisResponseTargetEnvironment.InternalStaging };

    public RunFcInternalUserConstraintCheckUseCaseTests()
    {
        ...
        var userPrincipal = UserFactory.CreateInternalUserIdentityProviderClaimsPrincipal(localAccountId:Guid.NewGuid());
        _user = new InternalUser(userPrincipal);
    ...
    private RunFcInternalUserConstraintCheckUseCase CreateSut(
        bool landInformationSearchIsSuccess = true)
    {
        ...
        return new RunFcInternalUserConstraintCheckUseCase(constraintCheckerService,
            new NullLogger<RunFcInternalUserConstraintCheckUseCase>());
    }
}
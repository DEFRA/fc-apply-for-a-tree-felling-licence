...
public class AddAdminOfficerTests
{
        ...
        Guid adminOfficerId)
    {
        ...
        MockAdminHubService
            .Setup(x => x.AddAdminOfficerAsync(It.IsAny<AddAdminOfficerToAdminHubRequestModel>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(UnitResult.Success<ManageAdminHubOutcome>());

        var result = await sut.AddAdminOfficerAsync(input, user, CancellationToken.None);

        Assert.True(result.IsSuccess);

        MockAdminHubService
            .Verify(x => x.AddAdminOfficerAsync(It.Is<AddAdminOfficerToAdminHubRequestModel>(x =>
        ...
        MockAuditService.Verify(x => x.PublishAuditEventAsync(It.Is<AuditEvent>(a =>
                a.EventName == AuditEvents.AddAdminOfficerToAdminHub
    ...
    public async Task WhenServiceReturnsOtherError(ManageAdminHubOutcome outcome)
    {
        ...
        MockAdminHubService.VerifyNoOtherCalls();
        MockAuditService.Verify(x => x.PublishAuditEventAsync(It.Is<AuditEvent>(a =>
                a.EventName == AuditEvents.AddAdminOfficerToAdminHubFailure
                && a.ActorType == ActorType.InternalUser
                && a.UserId == RequestContextUserId
                && a.SourceEntityId == adminHubId
                && a.SourceEntityType == SourceEntityType.AdminHub

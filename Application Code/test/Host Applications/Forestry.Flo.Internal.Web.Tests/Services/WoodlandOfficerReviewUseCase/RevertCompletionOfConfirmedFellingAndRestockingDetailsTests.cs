...
public class RevertCompletionOfConfirmedFellingAndRestockingDetailsTests : WoodlandOfficerReviewUseCaseTestsBase<Web.Services.FellingLicenceApplication.WoodlandOfficerReview.WoodlandOfficerReviewUseCase>
{
    [Theory, AutoMoqData]
    public async Task RevertCompletionOfConfirmedFellingAndRestockingDetails_WhenServiceReturnsError(
        Guid applicationId,
        Guid userId,
        string error)
    {
        ...
        // Act
        var result = await sut.RevertCompletionOfConfirmedFellingAndRestockingDetailsAsync(applicationId, user, CancellationToken.None);

        // Assert
        Assert.False(result.IsSuccess);
        Assert.Equal(error, result.Error);

        UpdateWoodlandOfficerReviewService.Verify(x => x.HandleConfirmedFellingAndRestockingChangesAsync(applicationId, userId, false, It.IsAny<CancellationToken>(), It.IsAny<bool>()), Times.Once);
    ...
    [Theory, AutoMoqData]
    public async Task RevertCompletionOfConfirmedFellingAndRestockingDetails_WhenServiceReturnsSuccess(
        Guid applicationId,
        Guid userId)
    {
        // Arrange

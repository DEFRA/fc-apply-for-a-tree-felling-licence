...
public class GetSiteVisitAddEvidenceModelTests : WoodlandOfficerReviewUseCaseTestsBase<SiteVisitUseCase>
{
    [Theory, AutoData]
    public async Task ShouldReturnFailureWhenUnableToRetrieveSiteVisit(
        Guid applicationId,
        string hostingPage)
    {
        var sut = CreateSut();
        ...
        var result = await sut.GetSiteVisitEvidenceModelAsync(applicationId, hostingPage, CancellationToken.None);
    ...
    [Theory, AutoData]
    public async Task ShouldReturnFailureWhenNoSiteVisitExistsYet(
        Guid applicationId,
        string hostingPage)
    {
        var sut = CreateSut();
        WoodlandOfficerReviewService
            .Setup(x => x.GetSiteVisitDetailsAsync(It.IsAny<Guid>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(Result.Success(Maybe<SiteVisitModel>.None));
        ...
        List<ActivityFeedItemModel> siteVisitComments)
    {
        ...
        var expectedMetadata = siteVisit.SiteVisitAttachments.Select(a => new SiteVisitEvidenceMetadataModel
        {
            FileName = a.FileName,
        ...
        Assert.Equivalent(expectedMetadata, result.Value.SiteVisitEvidenceMetadata);

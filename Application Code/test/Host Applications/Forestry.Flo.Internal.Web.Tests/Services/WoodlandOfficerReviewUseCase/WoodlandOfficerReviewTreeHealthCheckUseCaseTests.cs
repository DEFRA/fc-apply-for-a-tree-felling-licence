...
public class WoodlandOfficerReviewTreeHealthCheckUseCaseTests
{
    ...
    private readonly Mock<IUpdateWoodlandOfficerReviewService> _updateWoodlandOfficerReviewServiceMock = new();
    private readonly Mock<IAuditService<WoodlandOfficerTreeHealthCheckUseCase>> _auditServiceMock = new();
    ...
    public async Task GetViewModelWhenUnableToFindApplication(Guid applicationId, string error)
    {
        ...
        var result = await sut.GetTreeHealthCheckWoodlandOfficerViewModelAsync(applicationId, CancellationToken.None);
        ...
        _updateWoodlandOfficerReviewServiceMock.VerifyNoOtherCalls();
    ...
    [Theory, AutoMoqData]
    public async Task ConfirmTreeHealthCheckedWhenServiceReturnsFailure(Guid applicationId, Guid userId, bool isConfirmed, string error)
    {
        var sut = CreateSut();

        var user = GetInternalUser(userId);

        _updateWoodlandOfficerReviewServiceMock
            .Setup(x => x.ConfirmTreeHealthCheckAsync(It.IsAny<Guid>(), It.IsAny<Guid>(), It.IsAny<bool>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(Result.Failure(error));

        var result = await sut.ConfirmTreeHealthIssuesAsync(applicationId, user, isConfirmed, CancellationToken.None);
        Assert.True(result.IsFailure);
        ...
        _updateWoodlandOfficerReviewServiceMock
            .Verify(x => x.ConfirmTreeHealthCheckAsync(applicationId, userId, isConfirmed, It.IsAny<CancellationToken>()), Times.Once);
        _updateWoodlandOfficerReviewServiceMock.VerifyNoOtherCalls();

        _auditServiceMock.Verify(s =>
            s.PublishAuditEventAsync(It.Is<AuditEvent>(
                    e => e.EventName == AuditEvents.TreeHealthWoCheckCompletedFailure
                         && e.UserId == userId
    ...
    [Theory, AutoMoqData]
    public async Task ConfirmTreeHealthCheckedWhenServiceReturnsSuccess(Guid applicationId, Guid userId, bool isConfirmed)
    {
        var sut = CreateSut();
        ...
        _auditServiceMock.Verify(s =>
            s.PublishAuditEventAsync(It.Is<AuditEvent>(
                    e => e.EventName == AuditEvents.TreeHealthWoCheckCompleted
                         && e.UserId == userId
                         && e.ActorType == ActorType.InternalUser
                         && e.SourceEntityId == applicationId
                         && e.SourceEntityType == SourceEntityType.FellingLicenceApplication
    ...
    private WoodlandOfficerTreeHealthCheckUseCase CreateSut()
    {
        _getFlaServiceMock.Reset();
        _woodlandOwnerServiceMock.Reset();
        _updateWoodlandOfficerReviewServiceMock.Reset();
        ...
        return new WoodlandOfficerTreeHealthCheckUseCase(

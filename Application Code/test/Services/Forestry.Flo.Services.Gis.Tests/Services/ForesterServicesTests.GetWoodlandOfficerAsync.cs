...
{
    [Fact]
    public async Task GetWoodlandOfficerAsyncFailsWhenUnableToGetCentrepoint()
    {
        ...
        var result = await sut.GetWoodlandOfficerAsync(_polygons.Select(p => p.GetGeometrySimple()).ToList(), CancellationToken.None);

        Assert.True(result.IsFailure);
    }

    [Fact]
    public async Task GetWoodlandOfficerAsyncSuccess()
    {

...
public class UpdateWoodlandOfficerReviewServiceSetPublicRegisterExemptTests: UpdateWoodlandOfficerReviewServiceTestsBase
{
    [Theory, AutoData]
    ...
    [Theory, AutoData]
    public async Task ReturnsFailureWhenAlreadyPublishedToPublicRegister(
        Guid applicationId, 
        Guid userId,
        string reason)
    {
        ...
        var existing = new PublicRegister
        {
            FellingLicenceApplicationId = applicationId,
            ConsultationPublicRegisterPublicationTimestamp = DateTime.UtcNow.AddDays(-1)
        };
        ...
        FellingLicenceApplicationRepository
            ...
            .ReturnsAsync(Maybe<PublicRegister>.From(existing));
    ...
    [Theory, AutoData]
    public async Task CanSaveExemptionWhenNoPublicRegisterEntityOrWoodlandOfficerReviewEntityExists(
        Guid applicationId, 
        Guid userId,
        string reason)
    {
    ...
    [Theory, AutoData]
    public async Task CanSaveExemptionWhenPublicRegisterAndWoodlandOwnerReviewEntitiesExist(
        Guid applicationId, 
        Guid userId,
        string reason)
    {
    ...
    [Theory, AutoData]
    public async Task DoesNotSaveChangesAsExemptIfPublicRegisterAlreadyMatchesGivenValues(
        Guid applicationId,
        Guid userId,
        string reason)
    {
        ...
        var existingPr = new PublicRegister
        {
            ...
            WoodlandOfficerConsultationPublicRegisterExemptionReason = reason
        };
        ...
        Assert.True(result.IsSuccess);
        Assert.False(result.Value);  // false - no changes were made
    ...
    [Theory, AutoData]
    public async Task DoesNotSaveChangesAsNotExemptIfPublicRegisterAlreadyMatchesGivenValues(
        Guid applicationId,
        Guid userId)
    {
        var sut = CreateSut();
        ...
        var result = await sut.SetPublicRegisterExemptAsync(applicationId, userId, false, null, CancellationToken.None);

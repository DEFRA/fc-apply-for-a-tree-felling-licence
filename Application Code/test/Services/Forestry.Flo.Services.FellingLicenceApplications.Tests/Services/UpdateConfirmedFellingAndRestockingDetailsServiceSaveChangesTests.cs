...
public class UpdateConfirmedFellingAndRestockingDetailsServiceSaveChangesTests
{
    ...
    public async Task ConfirmedFellingAndRestockingDetailsAmended_WhenAllDataRetrieved(bool emptyConfirmed)
    {
        // setup
        ...
        var fla = ConstructFellingLicenceApplication(
            emptyConfirmed,
            userId);
        var subPropCompartment = fla!.SubmittedFlaPropertyDetail!.SubmittedFlaPropertyCompartments!.First();

        _fellingLicenceApplicationsContext!.Add(fla);
        await _fellingLicenceApplicationsContext.SaveChangesAsync();

        var detailModelList = ConstructConfirmedFellingAndRestockingDetailModels(fla, 5);

        var result = await sut.SaveChangesToConfirmedFellingAndRestockingAsync(
            fla.Id,
            userId,
            detailModelList,
        ...
        // assert result is success and database contains fla
        ...
        {
            var newCompartmentData = detailModelList.Find(x => x.CompartmentId == compartment.CompartmentId);

            // assert confirmed felling details have been updated 
            foreach (var updatedFellingDetail in compartment.ConfirmedFellingDetails)
            {
                if (!emptyConfirmed) { 
                    Assert.NotNull(newCompartmentData);
                    var newFelling = newCompartmentData!.ConfirmedFellingDetailModels.Where(x => x.ConfirmedFellingDetailsId == updatedFellingDetail.Id).FirstOrDefault();
                    Assert.NotNull(newFelling);
                    Assert.Equal(newFelling?.ConfirmedFellingSpecies, updatedFellingDetail.ConfirmedFellingSpecies);
                    ...
                    // assert confirmed restocking details have been updated 
                    foreach (var updatedRestock in updatedFellingDetail.ConfirmedRestockingDetails)
                    {
                        var newRestock = newFelling!.ConfirmedRestockingDetailModels.Where(x => x.ConfirmedRestockingDetailsId == updatedRestock.Id).FirstOrDefault();
                        Assert.Equal(newRestock?.ConfirmedRestockingSpecies, updatedRestock.ConfirmedRestockingSpecies);
                        Assert.Equal(newRestock?.Area, updatedRestock.Area);
                        Assert.Equal(newRestock?.PercentageOfRestockArea, updatedRestock.PercentageOfRestockArea);
                        Assert.Equal(newRestock?.RestockingDensity, updatedRestock.RestockingDensity);
                        Assert.Equal(newRestock?.RestockingProposal, updatedRestock.RestockingProposal);
                        Assert.Equal(newRestock?.PercentNaturalRegeneration, updatedRestock.PercentNaturalRegeneration);
                        Assert.Equal(newRestock?.PercentOpenSpace, updatedRestock.PercentOpenSpace);
                    }
                }
        ...
        // assert database has no orphaned entries for confirmed felling/restocking details

        Assert.Equal(storedFla.SubmittedFlaPropertyDetail!.SubmittedFlaPropertyCompartments!.Count,
            fla.SubmittedFlaPropertyDetail!.SubmittedFlaPropertyCompartments!.Count);

        Assert.Equal(fla.SubmittedFlaPropertyDetail.SubmittedFlaPropertyCompartments.Count, _fellingLicenceApplicationsContext.SubmittedFlaPropertyCompartments.Count());
        Assert.Equal(fla.SubmittedFlaPropertyDetail.SubmittedFlaPropertyCompartments.Count, _fellingLicenceApplicationsContext.SubmittedFlaPropertyCompartments.Count());
    ...
    public async Task ShouldReturnFailure_WhenCompartmentIsNotFound()
    {
        // setup
        ...
        var detailModelList = ConstructConfirmedFellingAndRestockingDetailModels(fla, 3);

        var randomId = Guid.NewGuid();

        detailModelList[0].CompartmentId = randomId;

        _fellingLicenceApplicationsContext!.Add(fla);
        await _fellingLicenceApplicationsContext.SaveChangesAsync();

        for (int i = 0; i < detailModelList.Count; i++)
        {
            var detailModel = detailModelList[i];
            detailModel.ConfirmedFellingDetailModels.FirstOrDefault()!.ConfirmedFellingDetailsId = fla.SubmittedFlaPropertyDetail!
                .SubmittedFlaPropertyCompartments![i].ConfirmedFellingDetails.First().Id;
        }
        ...
        _audit.Verify(v => v.PublishAuditEventAsync(
            It.Is<AuditEvent>(e =>
                ...
                {
                    Section = "Save Changes Confirmed Felling/Restocking Details",
                    Error = $"Unable to retrieve submitted property compartment, compartment id {randomId}"
                }, _options)),
            CancellationToken.None), Times.Once);
    }
    ...
    public async Task ShouldReturnFailure_WhenUserIsNotPermitted()
    {
        // setup

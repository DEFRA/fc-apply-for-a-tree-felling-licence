...
public partial class WoodlandOfficerReviewController : Controller
{
    ...
    [HttpGet]
    public async Task<IActionResult> FellingAndRestocking(
        Guid id,
        ...
        CancellationToken cancellationToken)
    {
        ...
        var model = await useCase.GetConfirmedFellingAndRestockingDetailsAsync(id, user, cancellationToken);
    ...
    [HttpPost]
    public async Task<IActionResult> UndoChanges(ConfirmedFellingRestockingDetailsModel model)
    {
        return RedirectToAction("FellingAndRestocking", new { id = model.ApplicationId });
    }

    [HttpPost]
    public async Task<IActionResult> ImportProposedFellingAndRestocking(
        ...
        CancellationToken cancellationToken)
    {
        ...
        var importResult = await useCase.ImportProposedFellingAndRestockingAsync(
            model.ApplicationId,
            user,
            cancellationToken);
        ...
        {
            this.AddErrorMessage("Something went wrong importing the proposed felling and restocking details, please try again");
            return RedirectToAction("FellingAndRestocking", new { id = model.ApplicationId });
        }

        this.AddConfirmationMessage("Proposed felling and restocking details were imported successfully");
        return RedirectToAction("FellingAndRestocking", new { id = model.ApplicationId });
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var compartmentModel in model.Compartments)
        {
            foreach (var cfd in compartmentModel.ConfirmedFellingDetails)
            {
                cfd.ConfirmedFellingSpecies = cfd.ConfirmedFellingSpecies.Where(z => !z.Deleted).ToArray();
                foreach (var crd in cfd.ConfirmedRestockingDetails)
                {
                    crd.ConfirmedRestockingSpecies = crd.ConfirmedRestockingSpecies.Where(z => !z.Deleted).ToArray();
                }
                
                // temporary setting until FLOV2-1322 is complete and this page is removed
                cfd.IsRestocking = cfd.ConfirmedRestockingDetails.Length > 0;
                cfd.NoRestockingReason = cfd.IsRestocking == true 
                    ? null 
                    : "Not required";
                cfd.IsTreeMarkingUsed = cfd.TreeMarking is not null;
            }
            ValidateModel(compartmentModel, _fellingAndRestockingDetailsModelValidator, true);
        }

        if (!ModelState.IsValid)
        {
            var confirmedDetails =
                await useCase.GetConfirmedFellingAndRestockingDetailsAsync(
                    model.ApplicationId,
                    user,
                    cancellationToken);

            if (confirmedDetails.IsFailure)
            {
                return RedirectToAction("Error", "Home");
            }

            model.FellingLicenceApplicationSummary = confirmedDetails.Value.FellingLicenceApplicationSummary!;
            model.Breadcrumbs = confirmedDetails.Value.Breadcrumbs;
            
            //This is a bit of a hack but we don't have any control over how the standard asp-validation-summary div html orders the
            //errors so we have to do it by hand
            var sortedErrors = ModelState
                .SelectMany(m => m.Value?.Errors.Select(x => x.ErrorMessage) ?? new List<string>(0))
                .OrderBy(x => x.Split(' ')[1])  // all error messages start "Compartment <cpt name> - etc etc"...
                .ToList();
            ViewBag.SortedErrors = sortedErrors;

            return View(model);
        }

        var result = await useCase.SaveConfirmedFellingAndRestockingDetailsAsync(model, user, cancellationToken);

        if (result.IsFailure)
        {
            this.AddErrorMessage("Something went wrong saving the confirmed felling and restocking details, please try again");
            return RedirectToAction("FellingAndRestocking", new { id = model.ApplicationId });
        }

        this.AddConfirmationMessage("Confirmed felling and restocking details saved");
        return RedirectToAction("FellingAndRestocking", new { id = model.ApplicationId });
    }

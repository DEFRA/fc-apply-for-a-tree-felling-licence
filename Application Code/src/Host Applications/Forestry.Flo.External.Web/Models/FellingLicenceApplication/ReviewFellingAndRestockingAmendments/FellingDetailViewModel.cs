...
public class FellingDetailViewModel : CompartmentConfirmedFellingRestockingDetailsModelBase
{
    ...
    /// <summary>
    /// Gets or sets a summary of the restocking compartments associated with this felling operation.
    /// </summary>
    public string? RestockingCompartments { get; set; }

    /// <summary>
    /// Gets or sets a summary of the restocking operations in this compartment, relating to this felling operation.
    /// </summary>
    /// <remarks>
    /// This is exclusively for restocking operations related to this felling operation, else this data could be duplicated.
    /// </remarks>
    public string? RestockingOperationsInCompartment { get; set; }
    ...
    public FellingDetailViewModel(ConfirmedFellingDetailViewModel model, CompartmentConfirmedFellingRestockingDetailsModelBase compartment)
    {
        ...
        CompartmentId = compartment.SubmittedFlaPropertyCompartmentId;
        ...
        RestockingOperationsInCompartment = string.Join(", ", model.ConfirmedRestockingDetails
            .Where(x => x.RestockingCompartmentId == CompartmentId)
            .Select(x => x.RestockingProposal?.GetDisplayName()));
    }

    /// <summary>
    /// Initializes a new instance of the <see cref="FellingDetailViewModel"/> class using a <see cref="Flo.Services.FellingLicenceApplications.Models.WoodlandOfficerReview.ProposedFellingDetailModel"/>
    /// and a <see cref="CompartmentConfirmedFellingRestockingDetailsModelBase"/> compartment.
    /// </summary>
    /// <param name="model">The proposed felling detail model containing felling details.</param>
    /// <param name="compartment">The compartment details associated with the felling operation.</param>
    public FellingDetailViewModel(Flo.Services.FellingLicenceApplications.Models.WoodlandOfficerReview.ProposedFellingDetailModel model, CompartmentConfirmedFellingRestockingDetailsModelBase compartment)
    {
        AreaToBeFelled = model.AreaToBeFelled;
        OperationType = model.OperationType;
        FellingSpecies = model.ListFellingSpecies();
